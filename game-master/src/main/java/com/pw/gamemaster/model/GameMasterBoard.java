...
public class GameMasterBoard extends Board {
		...
		if (this.getField(playerDTO.getPosition()).cell.playerGuids != null) {
			...
			if (playerDTO.playerTeamColor == TeamColor.BLUE) {
				if (playerDTO.playerPosition.getY() < this.boardHeight - this.goalAreaHeight) {
					this.cellsGrid[playerDTO.getPosition().getX()][playerDTO.getPosition().getY()]
							.setPlayerGuids(playerDTO.playerGuid.toString());
	...
	public Position placePlayer(PlayerDTO playerDTO) {
		if (this.getField(playerDTO.getPosition()) == null)
			return null;
		if (this.getField(playerDTO.getPosition()).cell.getplayerGuids() != null) {
			return null;
		}
		this.cellsGrid[playerDTO.playerPosition.getX()][playerDTO.playerPosition.getY()]
				.setPlayerGuids((playerDTO.playerGuid.toString()));
	...
	public List<Field> discover(Position position) {
		...
		for (int i = -1; i <= 1; i++) {
			for (int j = -1; j <= 1; j++) {
				...
				if (new_pos.getX() < this.boardWidth && new_pos.getX() > 0 && new_pos.y > 0
						&& new_pos.getY() < this.boardHeight) {
					...
					if (min_dist == this.boardWidth + this.boardHeight)
						min_dist = -1;
